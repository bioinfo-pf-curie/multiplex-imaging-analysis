process computeMasks {
  ...
  memory {params.segmentation.name == 'cellpose'? MemoryUnit.of(Math.max(Math.min(meta.imgSize * 0.6, params.maxMemory.size), params.minMemory.size).toLong()) : Math.min(params.maxMemory.size, (64.GB).getBytes())}
