process computeMasks {
  ...
  memory {params.segmentation.name == 'cellpose'? MemoryUnit.of(Math.max(Math.min(meta.imgSize * 1.2, params.maxMemory.size), params.minMemory.size).toLong()) : Math.min(params.maxMemory.size, (128.GB).getBytes())}
